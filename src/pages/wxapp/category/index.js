...
import { Space, Button, Table, Divider, Typography, message, Modal, Form, Input, Popconfirm, Cascader } from 'antd'
...
const Category = () => {
    ...
    const getCateList = async () => {
        ...
        let resp = await httpUtils.get('/admin/cate/front/list')
    ...
    const [backCateOption, setBackCateOption] = useState([])

    const getBackCateList = async () => {
        let resp = await httpUtils.get('/admin/cate/list')
        let wrap = wrapBackCateList(resp.children)
        console.log(wrap)
        setBackCateOption(wrap)
    }

    const wrapBackCateList = cates => {
        let temp = []
        cates.map(item => {
            let obj = {
                value: item.tree.id,
            ...
            if (item.children && item.children.length > 0) {
                obj.children = wrapBackCateList(item.children)
            }
            temp.push(obj)
        })
    ...
    const handleCateModalOk = async () => {
        ...
        if (cateModalData.id) {
            let data = {
                ...
                backCates: cateModalData.backCates || []
            }
            await httpUtils.post('/admin/cate/front/update', data)
        } else {
            let data = {
            ...
            await httpUtils.post('/admin/cate/front/add', data)
        }
            ...
            >
                <Form size="large">
                    ...
                    >
                        <Cascader 
                            size="middle"
                            options={backCateOption} 
                            onChange={e => {
                                setCateModalData({
                                    ...cateModalData,
                                    backCates: [e[e.length - 1]]
                                })
                            }} 
                            placeholder="请关联后台分类" 
                        />
